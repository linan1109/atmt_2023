...
def get_args():
    ...
    parser.add_argument('--n-best', default=1, type=int, help='number of best outputted')
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                node = BeamSearchNode(searches[i], emb, lstm_out, final_hidden, final_cell,
                                      mask, torch.cat((go_slice[i], next_word)), log_p, 1, diversity_penalty=args.gamma*j)
                ...
                for j in range(args.beam_size):
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:
                        node = BeamSearchNode(
                            ...
                            next_word)), node.logp, node.length, diversity_penalty=args.gamma*j
                            )
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        node = BeamSearchNode(
                            search, node.emb, node.lstm_out, node.final_hidden,
                            node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                            next_word)), node.logp + log_p, node.length + 1, diversity_penalty=args.gamma*j
                            )
        ...
        n_best_lists = [search.get_n_best(args.n_best) for search in searches]
        # Segment into sentences
        for k in range(args.n_best):
            all_hyps_k = {}
            best_sents = torch.stack([n_best_list[k].sequence[1:].cpu() for n_best_list in n_best_lists])
            ...
            for ii, sent in enumerate(output_sentences):
                all_hyps_k[int(sample['id'].data[ii])] = sent
            all_hyps[k] = all_hyps_k


    for k in range(args.n_best):
        # Write to file
        if args.output is not None:
            with open(args.output[:-4] + "." + str(k+1) + args.output[-4:], 'w') as out_file:
                for sent_id in range(len(all_hyps[k].keys())):
                    out_file.write(all_hyps[k][sent_id] + '\n')
